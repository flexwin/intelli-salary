        ...
        if (files) {
            ...
            XLSX.save(data, (err, newDoc) => {
                ...
                for (var i = 0; i < keys.length; i++) {
                    ...
                    if (!_hasEMail && reg.isEmail(values[i])) {
                        _hasEMail = true
                    }
                }
                // 邮箱固定显示
                ...
                if (!_hasEMail) {
                    ...
                    options.detail = `没有发现员工工资表中包含[邮箱]列，继续处理或者选择包含员工[身份证]和[邮箱]列的文件？`;
                    options.buttons = ['继续处理', '选择文件'];
...
autoUpdater.on('check-for-available-update', function () {
    checkForUpdates()
});
